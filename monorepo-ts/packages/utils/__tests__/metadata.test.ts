...
describe('Metadata', () => {
    describe('constructor', () => {
        ...
        it('creates metadata with provided options', () => {
            ...
            expect(metadata.description).toBe('Test metadata instance');
            expect(metadata.annotations).toEqual({
                type: 'test',
            });
            expect(metadata.labels).toEqual(['label1', 'label2']);
    ...
    describe('annotation methods', () => {
        ...
        it('sets and gets annotations', () => {
            ...
            expect(metadata.getAnnotation('key1')).toBe('value1');
            expect(metadata.hasAnnotation('key1')).toBe(true);
        });

        it('removes annotations', () => {
            metadata.setAnnotation('key1', 'value1');

            expect(metadata.removeAnnotation('key1')).toBe(true);
            expect(metadata.hasAnnotation('key1')).toBe(false);
            expect(metadata.removeAnnotation('nonexistent')).toBe(false);
        });

        it('updates timestamp when annotation is modified', async () => {
            const initialUpdated = metadata.getTimestamp('updated');

            await new Promise(resolve => setTimeout(resolve, 1));
            metadata.setAnnotation('key1', 'value1');
            const afterSet = metadata.getTimestamp('updated');

            expect(afterSet).toBeGreaterThan(initialUpdated || 0);

            await new Promise(resolve => setTimeout(resolve, 1));
            metadata.removeAnnotation('key1');
            const afterRemove = metadata.getTimestamp('updated');

            expect(afterRemove).toBeGreaterThan(afterSet || 0);
        });
    });
    ...
    describe('label methods', () => {
        ...
        it('removes labels', () => {
            ...
            expect(metadata.removeLabel('label1')).toBe(true);
            expect(metadata.labels).toEqual(['label2']);
            expect(metadata.removeLabel('nonexistent')).toBe(false);
        });
    });
    ...
    describe('tag methods', () => {
        ...
        it('removes tags', () => {
            ...
            expect(metadata.removeTag('nonexistent')).toBe(false);
        });
    });
    ...
    describe('timestamp methods', () => {
        ...
        it('sets and gets timestamps', () => {
            ...
            expect(metadata.getTimestamp('custom')).toBe(timestamp);
        });

        it('removes timestamps', () => {
            metadata.setTimestamp('custom', Date.now());

            expect(metadata.removeTimestamp('custom')).toBe(true);
            expect(metadata.getTimestamp('custom')).toBeUndefined();
            expect(metadata.removeTimestamp('nonexistent')).toBe(false);
        });
    });
    ...
    describe('utility methods', () => {
        ...
        it('clones metadata', () => {
            ...
            expect(clone.description).toBe(metadata.description);
            expect(clone.annotations).toEqual(metadata.annotations);
            expect(clone.annotations).not.toBe(metadata.annotations);
        ...
        it('creates from JSON', () => {
            ...
            expect(restored.description).toBe(metadata.description);
            expect(restored.annotations).toEqual(metadata.annotations);
            expect(restored.labels).toEqual(metadata.labels);
    ...
    describe('search and filter methods', () => {
        ...
        it('matches labels', () => {
            expect(metadata.matchesLabels(['label1'])).toBe(true);
            expect(metadata.matchesLabels(['label1', 'label2'])).toBe(true);
            expect(metadata.matchesLabels(['label1', 'missing'])).toBe(false);
        });

        it('matches tags', () => {
            expect(metadata.matchesTags(['tag1'])).toBe(true);
            expect(metadata.matchesTags(['tag1', 'tag2'])).toBe(true);
            expect(metadata.matchesTags(['tag1', 'missing'])).toBe(false);
        });
