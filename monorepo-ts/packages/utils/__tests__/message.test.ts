...
describe('MessageBuilder', () => {
    ...
    describe('fluent interface methods', () => {
        ...
        beforeEach(() => {
            builder = new MessageBuilder(MessageStatuses.SUCCESS, {
                body: testBody,
            });
        });
...
describe('MessageUtils', () => {
    ...
    describe('metadata utilities', () => {
        ...
        it('checks if message has specific metadata key', () => {
            expect(MessageUtils.hasMetadata(messages[0], 'category')).toBe(
                true
            );
            expect(MessageUtils.hasMetadata(messages[0], 'nonexistent')).toBe(
                false
            );
        });

        it('gets metadata value', () => {
            expect(MessageUtils.getMetadata(messages[0], 'category')).toBe(
                'test'
            );
            expect(
                MessageUtils.getMetadata(messages[1], 'category')
            ).toBeUndefined();
        });
    });
