...
describe('MessageBuilder', () => {
    ...
    describe('constructor', () => {
        it('creates a message with required fields', () => {
            ...
            expect(message.timestamp).toBeCloseTo(
                Date.now(),
                -3
            );
        });
    ...
    describe('fluent interface methods', () => {
        let builder: MessageBuilder<{ message: string }>;
...
describe('MessageUtils', () => {
    ...
    describe('status checking methods', () => {
        it('identifies success messages', () => {
            expect(
                MessageUtils.isSuccess(messages[0])
            ).toBe(true);
            expect(
                MessageUtils.isSuccess(messages[1])
            ).toBe(false);
        });

        it('identifies error messages', () => {
            expect(MessageUtils.isError(messages[1])).toBe(
                true
            );
            expect(MessageUtils.isError(messages[0])).toBe(
                false
            );
        });
        ...
        it('identifies warning messages', () => {
            expect(
                MessageUtils.isWarning(messages[2])
            ).toBe(true);
            expect(
                MessageUtils.isWarning(messages[0])
            ).toBe(false);
        });

        it('identifies info messages', () => {
            expect(MessageUtils.isInfo(messages[3])).toBe(
                true
            );
            expect(MessageUtils.isInfo(messages[0])).toBe(
                false
            );
        });

        it('identifies debug messages', () => {
            expect(MessageUtils.isDebug(messages[4])).toBe(
                true
            );
            expect(MessageUtils.isDebug(messages[0])).toBe(
                false
            );
        });
        ...
        it('identifies critical messages', () => {
            expect(
                MessageUtils.isCritical(messages[5])
            ).toBe(true);
            expect(
                MessageUtils.isCritical(messages[0])
            ).toBe(false);
        });
    });
    ...
    describe('metadata utilities', () => {
        it('checks if message has metadata', () => {
            expect(
                MessageUtils.hasMetadata(messages[0])
            ).toBe(true);
            expect(
                MessageUtils.hasMetadata(messages[1])
            ).toBe(false);
        });

        it('checks if message has specific metadata key', () => {
            expect(
                MessageUtils.hasMetadata(
                    messages[0],
                    'category'
                )
            ).toBe(true);
    ...
    describe('filtering and sorting', () => {
        it('filters messages by status', () => {
            const successMessages =
                MessageUtils.filterByStatus(
                    messages,
                    MessageStatuses.SUCCESS
                );

            expect(successMessages).toHaveLength(1);
            expect(successMessages[0].status).toBe(
                MessageStatuses.SUCCESS
            );
        });
        ...
        it('filters messages by time range', () => {
            ...
            expect(
                filtered.every(
                    msg =>
                        msg.timestamp >= 1640995300000 &&
                        msg.timestamp <= 1640995500000
                )
            ).toBe(true);
        });

        it('sorts messages by timestamp ascending', () => {
            const sorted = MessageUtils.sortByTimestamp(
                messages,
                true
            );
    ...
    describe('utility operations', () => {
        const originalMessage = createSuccessMessage(
            'test',
            {
                metadata: { key: 'value' },
                id: 'original-id',
        ...
        it('clones messages', () => {
            ...
            expect(cloned.metadata).not.toBe(
        ...
        it('merges messages', () => {
            ...
            const merged = MessageUtils.merge(
                originalMessage,
                updates
            );
        ...
        it('formats messages without timestamp', () => {
            ...
            const formatted = MessageUtils.formatMessage(
                message,
                false
            );
