...
class Metadata<T = unknown> {
    ...
    // Annotation methods
    public setAnnotation(key: string, value: TagValue): void {
    ...
    // Timestamp methods
    public setTimestamp(key: string, timestamp: number = timestampNow()): void {
    ...
    public merge(other: Metadata<T>): Metadata<T> {
        ...
        merged.labels.push(
            ...other.labels.filter(label => !merged.hasLabel(label))
        );
    ...
    public matchesAnnotations(annotations: Record<string, TagValue>): boolean {
