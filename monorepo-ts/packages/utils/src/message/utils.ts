import { MessageStatuses, type ReturnMessage } from './types';

class MessageUtils {
  // Status checking methods
  public static isSuccess<T>(message: ReturnMessage<T>): boolean {
    return message.status === MessageStatuses.SUCCESS;
  }

  public static isError<T>(message: ReturnMessage<T>): boolean {
    return message.status === MessageStatuses.ERROR;
  }

  public static isWarning<T>(message: ReturnMessage<T>): boolean {
    return message.status === MessageStatuses.WARNING;
  }

  public static isInfo<T>(message: ReturnMessage<T>): boolean {
    return message.status === MessageStatuses.INFO;
  }

  public static isDebug<T>(message: ReturnMessage<T>): boolean {
    return message.status === MessageStatuses.DEBUG;
  }

  public static isCritical<T>(message: ReturnMessage<T>): boolean {
    return message.status === MessageStatuses.CRITICAL;
  }

  public static isUnknown<T>(message: ReturnMessage<T>): boolean {
  ...
  public static filterBySource<T>(
    ...
    return messages.filter(message => message.source === source);
  }
  ...
  public static filterByTimeRange<T>(
    ...
    return messages.filter(
      message => message.timestamp >= startTime && message.timestamp <= endTime
    );
  }
  ...
  public static getStatusSummary<T>(
    ...
    const summary = Object.values(MessageStatuses).reduce(
      (acc, status) => {
        acc[status] = 0;
        return acc;
      },
  ...
  public static clone<T>(message: ReturnMessage<T>): ReturnMessage<T> {
