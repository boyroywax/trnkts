...
export class PluginManager {
  ...
  enablePlugin(pluginId: string, config?: Record<string, any>): void {
  ...
  getActivePlugins(): PluginConfig[] {
    return Array.from(this.activeConfigs.values()).filter(config => config.enabled);
  }
  ...
  getPluginsByArea(area: string): Array<{ plugin: Plugin; config: PluginConfig }> {
