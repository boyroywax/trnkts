...
const ThemeSwitchTestComponent = () => {
  ...
  return (
    <div>
      ...
      <div data-testid="current-background">{theme.colors.background}</div>
      <button data-testid="toggle-theme" onClick={() => setIsDark(!isDark)}>
        Toggle Theme
      </button>
    </div>
  );
...
const PluginSystemTestComponent = () => {
  ...
  return (
    <div>
      ...
      <button data-testid="register-plugin" onClick={() => registerPlugin(testPlugin)}>
        Register Plugin
      </button>

      <button data-testid="register-another" onClick={() => registerPlugin(anotherPlugin)}>
        Register Another
      </button>

      <button data-testid="unregister-plugin" onClick={() => unregisterPlugin('test-plugin')}>
        Unregister Plugin
      </button>

      <button data-testid="enable-plugin" onClick={() => enablePlugin('test-plugin')}>
        Enable Plugin
      </button>

      <button data-testid="disable-plugin" onClick={() => disablePlugin('test-plugin')}>
        Disable Plugin
      </button>

      <button
        data-testid="update-config"
        onClick={() => updatePluginConfig('test-plugin', { newSetting: 'newValue' })}
      >
        Update Config
      </button>
      ...
      <div data-testid="active-plugin-list">
        {activePlugins.map(pluginConfig => (
          <div key={pluginConfig.id} data-testid={`active-${pluginConfig.id}`}>
            {pluginConfig.id} - {pluginConfig.enabled ? 'enabled' : 'disabled'}
          </div>
        ))}
      </div>
    </div>
...
describe('Plugin System Integration', () => {
  ...
  it('registers and manages multiple plugins', () => {
    ...
    expect(screen.getByTestId('plugin-test-plugin')).toHaveTextContent('Test Plugin');
    ...
    expect(screen.getByTestId('plugin-another-plugin')).toHaveTextContent('Another Plugin');
  });
  ...
  it('unregisters plugins and removes from active list', () => {
    ...
    expect(screen.queryByTestId('plugin-test-plugin')).not.toBeInTheDocument();
    expect(screen.queryByTestId('active-test-plugin')).not.toBeInTheDocument();
  });
  ...
  it('prevents duplicate plugin registration with warning', () => {
    ...
    expect(pluginsCount).toHaveTextContent('1');
    expect(consoleSpy).toHaveBeenCalledWith('Plugin with id "test-plugin" is already registered');
