...
const ThemeSwitchTestComponent = () => {
    ...
    return (
        <div>
            ...
            <div data-testid='current-primary'>
...
const PluginSystemTestComponent = () => {
    ...
    const anotherPlugin: Plugin = {
        ...
        version: '2.0.0',
        component: () => (
            <div>Another Plugin Component</div>
        ),
    };

    return (
        <div>
            <div data-testid='plugins-count'>
                {plugins.length}
            </div>
            <div data-testid='active-plugins-count'>
...
describe('Theme Switching Integration', () => {
    it('switches between light and dark themes', () => {
        ...
        const currentPrimary = screen.getByTestId(
            'current-primary'
        );
        const toggleButton =
...
describe('Plugin System Integration', () => {
    it('loads initial plugins from config', () => {
        ...
        const activePluginsCount = screen.getByTestId(
            'active-plugins-count'
        );
        expect(activePluginsCount).toHaveTextContent('1'); // One initial plugin from config

        const activePlugin = screen.getByTestId(
            'active-initial-plugin'
        );
        expect(activePlugin).toHaveTextContent(
            'initial-plugin - enabled'
        );
    });
    ...
    it('registers and manages multiple plugins', () => {
        ...
        const pluginsCount =
            screen.getByTestId('plugins-count');
        const registerButton = screen.getByTestId(
            'register-plugin'
        );
        const registerAnotherButton = screen.getByTestId(
            'register-another'
        );
        ...
        expect(
            screen.getByTestId('plugin-test-plugin')
        ).toHaveTextContent('Test Plugin');
        ...
        expect(
            screen.getByTestId('plugin-another-plugin')
        ).toHaveTextContent('Another Plugin');
    });
    ...
    it('enables and disables plugins correctly', () => {
        ...
        const enableButton =
            screen.getByTestId('enable-plugin');
        const disableButton = screen.getByTestId(
            'disable-plugin'
        );
        ...
        const activeTestPlugin = screen.getByTestId(
            'active-test-plugin'
        );
        expect(activeTestPlugin).toHaveTextContent(
            'test-plugin - enabled'
        );
    ...
    it('unregisters plugins and removes from active list', () => {
        ...
        const enableButton =
            screen.getByTestId('enable-plugin');
        const unregisterButton = screen.getByTestId(
            'unregister-plugin'
        );
        ...
        expect(
            screen.getByTestId('active-test-plugin')
        ).toBeInTheDocument();
    ...
    it('updates plugin configuration', () => {
        ...
        const updateConfigButton =
            screen.getByTestId('update-config');
