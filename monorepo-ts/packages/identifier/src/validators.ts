 ...
 */
function validateNanoid(
  value: string,
  expectedSize: number = 21
): ValidationResult {
...
function validateCuid(value: string): ValidationResult {
  ...
  if (!cuidRegex.test(value)) {
    return {
      isValid: false,
      error:
        'Invalid CUID format. Must start with "c" followed by alphanumeric characters.',
    };
  }
  ...
  if (!pattern) {
    ...
    if (!value || value.trim() !== value || value.length === 0) {
      return {
        isValid: false,
        error:
          'Invalid custom identifier. Must be non-empty with no leading/trailing whitespace.',
      };
    }
    return { isValid: true };
  }
