...
export class SwarmKeyManager {
  ...
  static parseKey(keyString: string): Uint8Array {
    ...
    if (lines.length !== 3 || lines[0] !== '/key/swarm/psk/1.0.0/' || lines[1] !== '/base16/') {
      throw new Error('Invalid swarm key format. Expected format:\n/key/swarm/psk/1.0.0/\n/base16/\n<hex-key>');
    }
   ...
   */
  static async loadOrGenerate(filePath: string): Promise<{ key: Uint8Array; generated: boolean }> {
   ...
   */
  static getKeyInfo(key: Uint8Array): { valid: boolean; length: number; header: string; keyHex: string } {
